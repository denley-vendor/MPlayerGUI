...
#define MPLAYER_CFG_bEnableMPlayerMonitor      "bEnableMPlayerMonitor"
#define MPLAYER_CFG_bEnableLogging             "bEnableLogging"
#define MPLAYER_CFG_bBypassWinVersionCheck     "bBypassWinVersionCheck"
#define MPLAYER_CFG_bAllowSendCmdWhilePaused   "bAllowSendCmdWhilePaused"
#define MPLAYER_CFG_nPipeBufferSize            "nPipeBufferSize"
...
#define MPLAYER_CFG_bShowControlsInFullscreen  "bShowControlsInFullscreen"
#define MPLAYER_CFG_nMPlayerMonitorThreshold   "nMPlayerMonitorThreshold"
#define MPLAYER_CFG_bRememberLastPlaybackPosition "bRememberLastPlaybackPosition"
...
#define MPLAYER_CFG_bEnableArgumentVolume      "bEnableArgumentVolume"
...
#define TIMER_DELAYED_VOLUME_SET   5
...
BEGIN_MESSAGE_MAP(CMainFrame, CFrameWnd)
	...
	ON_WM_MOUSEWHEEL()
	ON_COMMAND(ID_PLAY_SENDCOMMAND, OnPlaySendcommand)
	//}}AFX_MSG_MAP
...
void CMainFrame::Init(void)
{
  ...
  m_MPlayerGUIStatus.nSeekToPercent = -1;
  ...
  m_MPlayerGUIStatus.bBarsOnFullScreen = FALSE;
  ...
  // m_MPlayerGUICfg.bEnableMPlayerMonitor = TRUE;
  ...
  m_MPlayerGUICfg.bShowControlsInFullscreen = TRUE;
  // m_MPlayerGUICfg.nMPlayerMonitorThreshold = 10;
  m_MPlayerGUICfg.bRememberLastPlaybackPosition = FALSE;
  m_MPlayerGUICfg.nVolumeLoaded = 0;
  // m_MPlayerGUICfg.bEnableArgumentVolume = TRUE;
...
void CMainFrame::DoPlayPlay(void)
{
  ...
  if (nStatus == MPLAYER_STOPPED)
  {
    // Read and set Playback Position...
    if (m_MPlayerGUICfg.bRememberLastPlaybackPosition)
    {
      CPlaybackPosition MyPlaybackPosition;
      int nPlaybackPosition;
      nPlaybackPosition = MyPlaybackPosition.ReadInteger(m_MPlayerGUIStatus.strSource, -1);
      if (nPlaybackPosition != -1)
      {
        nPlaybackPosition -= 5; // TODO: Let user set this Rollback value

        if (nPlaybackPosition > 0)
        {
          m_MPlayer.SetPlaybackPosition(nPlaybackPosition);
        }
      }
    }
...
void CMainFrame::OnPlayStop() 
{
  ...
  if ((nStatus == MPLAYER_PLAYING) || (nStatus == MPLAYER_PAUSED) || (nStatus == MPLAYER_STARTING))
  {
    ...
    {
      CPlaybackPosition MyPlaybackPosition;
      if (m_MPlayerGUIStatus.strSource.GetLength() > 0)
      {
        if (m_MPlayerGUIStatus.strSource.Find("://") == -1) // Not URL
        {
          MyPlaybackPosition.WriteInteger(m_MPlayerGUIStatus.strSource, (int)m_MPlayer.GetTimePos());
        }
      }
    }
...
void CMainFrame::PlayerCleanUp(void)
{
  ...
  KillTimer(TIMER_DELAYED_VOLUME_SET);
...
void CMainFrame::OnPlayPause() 
{
  ...
  else if (nStatus == MPLAYER_STOPPED)
  {
    // Play from MPLAYER_STOPPED
    DoPlayPlay();
  }
}
...
void CMainFrame::OnFileCloseFile() 
{
  ...
  {
    CPlaybackPosition MyPlaybackPosition;
    MyPlaybackPosition.DeleteKey(m_MPlayerGUIStatus.strSource);
  }
...
LRESULT CMainFrame::OnSliderProgress(WPARAM wParam, LPARAM lParam)
{
  int nStatus = m_MPlayer.GetStatus();
  long lDuration;
...
LRESULT CMainFrame::OnSliderVolume(WPARAM wParam, LPARAM lParam)
{
  int nStatus = m_MPlayer.GetStatus();
...
void CMainFrame::OnOptionsPreferences() 
{
  ...
  PagePlayer.m_bShowControlsInFullscreen = m_MPlayerGUICfg.bShowControlsInFullscreen;
  PagePlayer.m_bRememberLastPlaybackPosition = m_MPlayerGUICfg.bRememberLastPlaybackPosition;
  ...
  PageAdvanced.m_bEnableMPlayerMonitor = m_MPlayer.GetEnableMPlayerMonitor();
  PageAdvanced.m_nMPlayerMonitorThreshold = m_MPlayer.GetMPlayerMonitorThreshold();
  PageAdvanced.m_bEnableLogging = m_MPlayer.GetEnableLogging();
  PageAdvanced.m_bEnableArgumentVolume = m_MPlayer.GetEnableArgumentVolume();

  bTemp = m_MPlayer.GetAllowSendCmdWhilePaused();
  PageAdvanced.m_bAllowSendCmdWhilePaused = bTemp;
  ...
  if (MySheet.DoModal() == IDOK)
  {
    ...
    m_MPlayerGUICfg.bShowControlsInFullscreen = PagePlayer.m_bShowControlsInFullscreen;
    m_MPlayerGUICfg.bRememberLastPlaybackPosition = PagePlayer.m_bRememberLastPlaybackPosition;
    ...
    m_MPlayer.SetEnableMPlayerMonitor(PageAdvanced.m_bEnableMPlayerMonitor);
    m_MPlayer.SetMPlayerMonitorThreshold(PageAdvanced.m_nMPlayerMonitorThreshold);

    // m_MPlayerGUICfg.bEnableLogging = PageAdvanced.m_bEnableLogging;
    // m_MPlayer.SetEnableLogging(m_MPlayerGUICfg.bEnableLogging);
    m_MPlayer.SetEnableLogging(PageAdvanced.m_bEnableLogging);

    m_MPlayer.SetEnableArgumentVolume(PageAdvanced.m_bEnableArgumentVolume);

    m_MPlayer.SetAllowSendCmdWhilePaused(PageAdvanced.m_bAllowSendCmdWhilePaused);
...
void CMainFrame::ReadIniSettings(void)
{
  ...
  m_MPlayerGUICfg.bShowControlsInFullscreen = MyAppSettings.ReadBoolean(MPLAYER_CFG_bShowControlsInFullscreen, TRUE);
  m_MPlayerGUICfg.bRememberLastPlaybackPosition = MyAppSettings.ReadBoolean(MPLAYER_CFG_bRememberLastPlaybackPosition, FALSE);
  ...
  m_MPlayer.SetEnableMPlayerMonitor(MyAppSettings.ReadBoolean(MPLAYER_CFG_bEnableMPlayerMonitor, TRUE));
  m_MPlayer.SetMPlayerMonitorThreshold(MyAppSettings.ReadInteger(MPLAYER_CFG_nMPlayerMonitorThreshold, 10));
  // m_MPlayerGUICfg.bEnableLogging = MyAppSettings.ReadBoolean(MPLAYER_CFG_bEnableLogging, TRUE);
  // m_MPlayer.SetEnableLogging(m_MPlayerGUICfg.bEnableLogging);
  m_MPlayer.SetEnableLogging(MyAppSettings.ReadBoolean(MPLAYER_CFG_bEnableLogging, TRUE));
  m_MPlayer.SetEnableArgumentVolume(MyAppSettings.ReadBoolean(MPLAYER_CFG_bEnableArgumentVolume, TRUE));

  m_MPlayerGUIStatus.bBypassWinVersionCheck = MyAppSettings.ReadBoolean(MPLAYER_CFG_bBypassWinVersionCheck, FALSE);

  m_MPlayer.SetAllowSendCmdWhilePaused(MyAppSettings.ReadBoolean(MPLAYER_CFG_bAllowSendCmdWhilePaused, FALSE));

  m_MPlayer.SetPipeBufferSize(MyAppSettings.ReadInteger(MPLAYER_CFG_nPipeBufferSize, 0));
...
void CMainFrame::WriteIniSettings(void)
{
  ...
  MyAppSettings.WriteBoolean(MPLAYER_CFG_bShowControlsInFullscreen, m_MPlayerGUICfg.bShowControlsInFullscreen);
  MyAppSettings.WriteBoolean(MPLAYER_CFG_bRememberLastPlaybackPosition, m_MPlayerGUICfg.bRememberLastPlaybackPosition);
  ...
  bTemp = m_MPlayer.GetEnableMPlayerMonitor();
  MyAppSettings.WriteBoolean(MPLAYER_CFG_bEnableMPlayerMonitor, bTemp);

  nTemp = m_MPlayer.GetMPlayerMonitorThreshold();
  MyAppSettings.WriteInteger(MPLAYER_CFG_nMPlayerMonitorThreshold, nTemp);

  bTemp = m_MPlayer.GetEnableLogging();
  MyAppSettings.WriteBoolean(MPLAYER_CFG_bEnableLogging, bTemp);

  bTemp = m_MPlayer.GetAllowSendCmdWhilePaused();
  MyAppSettings.WriteBoolean(MPLAYER_CFG_bAllowSendCmdWhilePaused, bTemp);

  bTemp = m_MPlayer.GetEnableArgumentVolume();
  MyAppSettings.WriteBoolean(MPLAYER_CFG_bEnableArgumentVolume, bTemp);
...
void CMainFrame::OnViewFullscreen() 
{
  ...
  if (m_MPlayerGUIStatus.bFullScreen == FALSE)
  {
    ...
    {
      ShowControlBar(&m_wndMyToolBar, FALSE, TRUE);
    }
    ...
    {
      ShowControlBar(&m_wndProgressDlgBar, FALSE, TRUE);
    }
  ...
  else
  {
    ...
    if (m_MPlayerGUIStatus.bToolbarVisible)
    {
      ShowControlBar(&m_wndMyToolBar, TRUE, TRUE);
    }
    else
    {
      ShowControlBar(&m_wndMyToolBar, FALSE, TRUE);
    ...
    if (m_MPlayerGUIStatus.bProgressBarVisible)
    {
      ShowControlBar(&m_wndProgressDlgBar, TRUE, TRUE);
    }
    else
    {
      ShowControlBar(&m_wndProgressDlgBar, FALSE, TRUE);
...
LRESULT CMainFrame::OnPlayListPlay(WPARAM wParam, LPARAM lParam)
{
...
LRESULT CMainFrame::OnPlaylistDlgHide(WPARAM wParam, LPARAM lParam)
{
  m_pPlaylistDlg->ShowWindow(SW_HIDE);
  return 1;
}
...
LRESULT CMainFrame::OnStartupOpenfile(WPARAM wParam, LPARAM lParam)
{
...
LRESULT CMainFrame::OnViewMPlayerPause(WPARAM wParam, LPARAM lParam)
{
  OnPlayPause();
  return 1;
}
...
LRESULT CMainFrame::OnViewMPlayerFullScreen(WPARAM wParam, LPARAM lParam)
{
  int nStatus = m_MPlayer.GetStatus();
...
LRESULT CMainFrame::OnMPlayerStarting(WPARAM wParam, LPARAM lPARAM)
{
  // Starting...
...
LRESULT CMainFrame::OnMPlayerStartPlaying(WPARAM wParam, LPARAM lParam)
{
  KillTimer(TIMER_CACHEFILL);
  // Pre-set volume
  ...
  SetTimer(TIMER_DELAYED_VOLUME_SET, 2000, NULL);
  ...
  */
  ...
  if (m_MPlayerGUIStatus.strSource.Find("://") == -1)
  {
  ...
  {
    strTitle = m_MPlayerGUIStatus.strSource;
    strName = m_MPlayerGUIStatus.strSource;
  }
...
LRESULT CMainFrame::OnMPlayerFailedToStart(WPARAM wParam, LPARAM lParam)
{
  PlayerCleanUp();
...
LRESULT CMainFrame::OnMPlayerStopped(WPARAM wParam, LPARAM lParam)
{
  PlayerCleanUp();
  m_SSDisabler.Restore();
...
LRESULT CMainFrame::OnMPlayerAutoStopped(WPARAM wParam, LPARAM lParam)
{
...
void CMainFrame::OnTimer(UINT nIDEvent) 
{
  switch (nIDEvent)
  {
    ...
    case TIMER_ONE_SECOND:
    {
      ...
      if (m_MPlayerGUICfg.bShowControlsInFullscreen)
      {
    ...
    case TIMER_DELAYED_VOLUME_SET:
    {
      // Delayed Volume set in case of slow system
      int nVolume = m_wndMyToolBar.GetSliderVolume();
      m_MPlayer.SetVolume(nVolume);
      KillTimer(TIMER_DELAYED_VOLUME_SET);
      break;
    ...
    case TIMER_FULLSCREENMOUSEHIDER:
    {
      ...
      if (!pView->IsContextMenuShown() && !m_MPlayerGUIStatus.bBarsOnFullScreen)
      {
...
void CMainFrame::UpdateProgress(void)
{
  ...
  if (nPercent != -1)
  {
    ...
    // Make sure the progress is synchronized
    nSliderPos = m_wndProgressDlgBar.GetSliderProgress();
    if (nSliderPos != m_MPlayerGUIStatus.nPercent)
    {
      m_wndProgressDlgBar.SetSliderProgress(m_MPlayerGUIStatus.nPercent);
    }
  }
}
...
LRESULT CMainFrame::OnLogDlgHide(WPARAM wParam, LPARAM lParam)
{
  m_pLogDlg->ShowWindow(SW_HIDE);
  return 1;
}
...
LRESULT CMainFrame::OnMPlayerLog(WPARAM wParam, LPARAM lParam)
{
  if (m_pLogDlg)
  {
    CString strLog;
...
void CMainFrame::ShowControlsInFullscreen(void)
{
  if (m_MPlayerGUIStatus.bFullScreen == TRUE)
  {
    CPoint p;
    GetCursorPos(&p);
    if (m_MPlayerGUIStatus.rFullScreenWindowRect.Height() - p.y < 100)
    {
      if (!m_MPlayerGUIStatus.bBarsOnFullScreen)
      {
        ShowControlBar(&m_wndMyToolBar, TRUE, TRUE);
        ShowControlBar(&m_wndStatusBar, TRUE, TRUE);
        ShowControlBar(&m_wndProgressDlgBar, TRUE, TRUE);
        // ::SetMenu(m_hWnd, m_hMenuDefault);
        m_MPlayerGUIStatus.bBarsOnFullScreen = TRUE;
      }
    }
    else
    {
...
void CMainFrame::OnPlaySendcommand() 
{
  CSendCommandDlg MySendCommandDlg;

  if (MySendCommandDlg.DoModal() == IDOK)
  {
    CString strCommand;
    MySendCommandDlg.GetCommand(strCommand);
