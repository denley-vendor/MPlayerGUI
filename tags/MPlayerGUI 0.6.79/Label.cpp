...
{
	// CRect (rc);                             // Comment out to remove flicker
	// GetWindowRect(rc);                      // Comment out to remove flicker
	RedrawWindow();

	// GetParent()->ScreenToClient(rc);        // Comment out to remove flicker
	// GetParent()->InvalidateRect(rc,TRUE);   // Comment out to remove flicker
	// GetParent()->UpdateWindow();            // Comment out to remove flicker
}
...
void CLabel::OnPaint() 
{
  ...
  if (m_strText == _T("")) // Add to remove flicker
  ...
  // Added by Jason, to add ellipsis at end of text
  if (m_bTextEllipsis)
...
// Added by Jason, to add ellipsis at end of text
CLabel& CLabel::SetTextEllipsis(BOOL bTextEllipsis)
{
  m_bTextEllipsis = bTextEllipsis;
