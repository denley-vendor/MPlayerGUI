...
int CMPlayer::Init(void)
{
  ...
  m_bPendingVolume = FALSE; // No Pending Volume to be set
...
int CMPlayer::CreateCmdPipe(void)
{
  ...
  if (!::CreatePipe(&hReadFromChild, &m_hWriteToParent, &m_saAttr, 8192))
  {
    // pipe failed
    return 0;
  }
...
void CMPlayer::Play(void)
{
  ...
  if (bStarted)
  {
    ...
    */
    ...
    AfxGetApp()->m_pMainWnd->PostMessage(UWM_MPLAYER_STARTING, 0, 0);
  }
  else
  {
    // Failed to start
    ...
    CleanUp();
    AfxGetApp()->m_pMainWnd->PostMessage(UWM_MPLAYER_FAILEDTOSTART, 0, 0);
  }
}
...
void CMPlayer::MakeupCmdLn(void)
{
  ...
  // m_strMPlayerConfig = _T("-quiet -slave -identify -colorkey 0x101010");
  AddArgument(_T("-quiet"));
  AddArgument(_T("-slave"));
  AddArgument(_T("-identify"));
  AddArgument(_T("-colorkey 0x101010"));
  ...
  if (nStatus == 1)
    ...
    if (CheckActivate() == 0)
      ...
      // SetTimer(TIMER_EIGHT_SECOND, 8000, NULL);
	    ...
	    m_nStatus = MPLAYER_PLAYING;
			AfxGetApp()->m_pMainWnd->PostMessage(UWM_MPLAYER_STARTPLAYING, 0, 0);
  	}
	}
...
void CMPlayer::Pause(BOOL bPause)
{
  if ((m_nStatus == MPLAYER_PLAYING) || (m_nStatus == MPLAYER_PAUSED))
  {
    ...
    else
    {
      ...
      // If user set volume when pausing, set the value now
      if (m_bPendingVolume)
      {
        CString strVolume;
        strVolume.Format("volume %d 1 pause", m_nVolume);
        Command(strVolume);
        m_bPendingVolume = FALSE;
      }
      else
      {
        Command("pause");
...
void CMPlayer::Stop(void)
{
  if ((m_nStatus == MPLAYER_PLAYING) || (m_nStatus == MPLAYER_PAUSED) || (m_nStatus == MPLAYER_STARTING))
  {
    ...
    AfxGetApp()->m_pMainWnd->PostMessage(UWM_MPLAYER_STOPPED, 0, 0);
    m_nStatus = MPLAYER_STOPPED;
  }
}
...
void CMPlayer::ParseMPlayerOutput(CString strMPlayerOutput)
{
  if (strMPlayerOutput.Find("ANS_TIME_POSITION") != -1)
  {
    ...
    strMPlayerOutput = strMPlayerOutput.Mid(18);
  ...
  if (m_nStatus == MPLAYER_STARTING)
  {
    ...
    {
      strMPlayerOutput = strMPlayerOutput.Mid(12);
    ...
    {
      m_strClipInfo_Clip = strMPlayerOutput.Mid(18);
      return;
    }

    if (strMPlayerOutput.Find("ID_CLIP_INFO_author") != -1)
    {
      m_strClipInfo_Author = strMPlayerOutput.Mid(20);
      return;
    }

    if (strMPlayerOutput.Find("ID_CLIP_INFO_copyright") != -1)
    {
      m_strClipInfo_Copyright = strMPlayerOutput.Mid(23);
      return;
    }

    if (strMPlayerOutput.Find(" Title:") != -1)
    {
      m_strClipInfo_Title = strMPlayerOutput.Mid(8);
      return;
    }
    ...
    {
      m_strClipInfo_Title = strMPlayerOutput.Mid(7);
      return;
    }

    if (strMPlayerOutput.Find(" Artist:") != -1)
    {
      m_strClipInfo_Artist = strMPlayerOutput.Mid(9);
      return;
    }
    ...
    {
      m_strClipInfo_Album = strMPlayerOutput.Mid(8);
      return;
    }

    if (strMPlayerOutput.Find(" Genre:") != -1)
    {
      m_strClipInfo_Genre = strMPlayerOutput.Mid(8);
      return;
    }

    if (strMPlayerOutput.Find("ID_LENGTH") != -1)
    {
      strMPlayerOutput = strMPlayerOutput.Mid(10);
    ...
    {
      m_strAudioInfo_Codec = strMPlayerOutput.Mid(15);
      return;
    }

    if (strMPlayerOutput.Find("ID_AUDIO_FORMAT") != -1)
    {
      m_strAudioInfo_Format = strMPlayerOutput.Mid(16);
      return;
    }

    if (strMPlayerOutput.Find("ID_AUDIO_BITRATE") != -1)
    {
      strMPlayerOutput = strMPlayerOutput.Mid(17);
    ...
    {
      strMPlayerOutput = strMPlayerOutput.Mid(14);
    ...
    {
      strMPlayerOutput = strMPlayerOutput.Mid(13);
    ...
    {
      m_strVideoInfo_Codec = strMPlayerOutput.Mid(15);
      return;
    }
    ...
    {
      strMPlayerOutput = strMPlayerOutput.Mid(15);
    ...
    {
      strMPlayerOutput = strMPlayerOutput.Mid(16);
...
void CMPlayer::CalculateVideoBitrate(void)
{
  if (m_bHaveVideo)
  {
    if (m_lVideoInfo_Bitrate == 0)
    {
      ...
      {
        m_lVideoInfo_Bitrate = lFileSize / m_lClipInfo_Length * 8;
...
void CMPlayer::CheckAutoStop(void)
{
  if (m_nStatus == MPLAYER_PLAYING)
    ...
    {
      AutoStop();
      AfxGetApp()->m_pMainWnd->PostMessage(UWM_MPLAYER_AUTOSTOPPED, 0, 0);
    }
	}
}
...
void CMPlayer::MPlayerMonitor()
{
  ...
  while (TRUE)
  {
    ...
    // Compare time, if more than 20 seconds the ReadMPlayerOutput() is not called
    // terminate the MPlayer process
    timeCurrent = CTime::GetCurrentTime();

    // m_timeLastReadCmd is set in ReadMPlayerOutput()
    if ((timeCurrent - m_timeLastReadCmd) > 20)
    {
...
void CMPlayer::SetDisableMPlayerMonitor(BOOL bDisableMPlayerMonitor)
{
  m_bDisableMPlayerMonitor = bDisableMPlayerMonitor;
}
